...
public class ZEDLiveLinkFusion : ModuleRules
{
    public ZEDLiveLinkFusion(ReadOnlyTargetRules Target) : base(Target)
    {
        string CudaSDKPath = "";
        string ZEDSDKPath = "";
    ...
    public void LoadWrapper(ReadOnlyTargetRules Target, string DirPath)
    {
        ...
        else if (Target.Platform == UnrealTargetPlatform.Linux)
        {
            ...
            RuntimeDependencies.Add("$(TargetOutputDir)/ZEDLiveLinkFusion/libsl_zed_c.so", Path.Combine(DirPath + "/linux/libsl_zed_c.so"));
        }
    }
}
